...
from loc_utils.tcga_dataset import get_dataset, get_dataset_alltcga
...
def prediction(model, 
    ...
    for it, dbatch in pbar:
        ...
        for ct in cancer_type:
            if ct not in cancer_spc_dict:
                cancer_spc_dict[ct] = []
            cancer_spc_dict[ct].extend(idx.cpu().numpy().tolist())
    ...
    for cancer_type in cancer_spc_dict:
        idx_list:list = cancer_spc_dict[cancer_type]
        idx_list.sort()
        if task_type =='surv':
            # all_pred_logits = torch.cat(all_pred_logits, dim=0)
            hazards = torch.sigmoid(all_pred_logits[idx_list])
            survival = torch.cumprod(1 - hazards, dim=1)
            risk = -torch.sum(survival, dim=1).detach().cpu().numpy()

            all_risk_scores = risk
            all_censorships = all_censorships[idx_list]
            all_event_times = all_survival_times[idx_list]
            c_index_result = concordance_index_censored((1 - all_censorships).astype(bool), all_event_times, all_risk_scores)
            c_index = c_index_result[0]
            report_metrics[cancer_type+'_c-index'] = c_index
...
def main(args):
    ...
    train_dataset, valid_dataset, test_dataset = get_dataset_alltcga(args.report_label_path, modalities=modality_config, 
                                                             task_type=model_config.obj.task_type,
                                                             img_select=model_config.obj.img_select,
                                                             cancer_types=model_config.obj.cancer_types)
